...
@Controller
@RequestMapping(value = UmMappings.PATIENTS)
public class PatientRecordController extends AbstractController<Patient> implements ISortingController<Patient> {
    ...
    @ResponseBody
    @Secured(Privileges.CAN_PATIENT_RECORD_READ)
    public List<Patient> findAllPaginated(@RequestParam(value = QueryConstants.PAGE) final int page, @RequestParam(value = QueryConstants.SIZE) final int size) {
    ...
    @ResponseBody
    @Secured(Privileges.CAN_PATIENT_RECORD_READ)
    public List<Patient> findAll(final HttpServletRequest request) {
    ...
    public void create(@RequestBody @Valid final Patient resource) {
